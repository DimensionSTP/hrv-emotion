...
class SliceBioPacData():
    def __init__(self, log_path, data_path, save_path):
    ...
    def get_ecg_start_time(self, data_file_name):
    ...
    def get_video_logs(self, log_file_name):
    ...
    def get_absolute_time_stamp(self, video_logs, ecg_start_time):
        datetimes = []
        datetime_format = "%H:%M:%S.%f"
        
        for i, video_log in enumerate(video_logs):
            # if i == 0:
            #     t = video_log.split(" ")[2][:-1]
            # else:
    ...
    def get_stimuli_types(self, video_logs):
        stimuli = ["reference"]
        for i in range(1, 5):
            stimuli.append(video_logs[i][9:13])
        return stimuli
    
    def get_data_file(self, data_file_name):
        data_file = pd.read_csv(
            f"{self.data_path}/{data_file_name}", header=None, delimiter="\t", skiprows=23
        )
        return data_file
    
    def get_subject_name(self, log_file_name, data_file_name):
...
if __name__ == "__main__":
    ...
    slice_biopac_data = SliceBioPacData(
        "./logs", "./data_all", "./data"
    )
    slice_biopac_data()